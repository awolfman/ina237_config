...
# Записываем калибровочное значение
        (stdin, stdout, stderr) = client.exec_command(f'{_set} -y {bus} {i2caddr[i]} {REG_SHUNT_CAL} {calibration_value} w')
        time.sleep(0.1)
# Проверяем, что всё записалось правильно
        (stdin, stdout, stderr) = client.exec_command(f'{_get} -y {bus} {i2caddr[i]} {REG_SHUNT_CAL} w')
        print("Проверяем паравильность записи калибровки:", stdout.read().decode() )
#
        (stdin, stdout, stderr) = client.exec_command(f'{_get} -y {bus} {i2caddr[i]} {REG_VBUS} w')
        raw_vbus = stdout.read().decode()
        vbus = int( ''.join(char for char in raw_vbus if char.isalnum()) , 16 )
        print ("Vin = {:.2f} V".format(vbus * 0.003125 ) )
#
        (stdin, stdout, stderr) = client.exec_command(f'{_get} -y {bus} {i2caddr[i]} {REG_CURRENT} w')
...
#
        (stdin, stdout, stderr) = client.exec_command(f'{_get} -y {bus} {i2caddr[i]} {REG_POWER} w')
...
#
        (stdin, stdout, stderr) = client.exec_command(f'{_get} -y {bus} {i2caddr[i]} {REG_DIETEMP} w')
